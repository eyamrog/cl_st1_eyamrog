...
# Usage: (my_env) eyamrog@Rog-ASUS:~/work/cl_st1_eyamrog$ nohup python cl_st1_ph5_test_eyamrog.py cl_st1_ph5_eyamrog cl_st1_ph5_eyamrog &
...
def main(input_directory, output_directory):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f"{output_directory}/chatgpt_review_test.log"),
    ...
    # Applying the function to the 'Text Paragraph' column with progress indication
    improved_texts = []
    for text in tqdm(df_scielo_preprint_preChatGPT_en['Text Paragraph'], desc='Processing texts'):
        improved_texts.append(improve_text(text))

    df_scielo_preprint_preChatGPT_en['Text Paragraph ChatGPT'] = improved_texts

    # Exporting each paragraph processed by ChatGPT to individual files for inspection
    for index, row in df_scielo_preprint_preChatGPT_en.iterrows():
        file_name = f"{output_directory}/{row['Text ID']}_{row['Section']}_{row['Paragraph']}_test_chatgpt.txt"
        with open(file_name, 'w', encoding='utf-8') as file:
            file.write(row['Text Paragraph ChatGPT'])

    # Exporting to a file
    df_scielo_preprint_preChatGPT_en.to_json(f"{output_directory}/scielo_chatgpt_erpp_pp_test.jsonl", orient='records', lines=True)
